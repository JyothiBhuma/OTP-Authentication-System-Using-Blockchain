...
// Twilio setup
const accountSid = "ACf36e8398ce69db8ef34de547a049641e";
const apiKeySid = "SKda86591977b60fe97de2e44a974ad17b";
const apiKeySecret = "CycBEGqXRlhoOmF3wPOEibn6wXygcRBn";
...
app.get('/otp', async (req, res) => {
    try {
        ...
        if (otp) {
            const recipientPhoneNumber = "+919392458441"; // The user's phone number
            const messageDetails = {
                body: `[OTP Auth] Your OTP is: ${otp}. Please do not share it with anyone.`,
                from: "+14198751778", // Your Twilio phone number
                to: recipientPhoneNumber,
            };

            const message = await client.messages.create(messageDetails);
            console.log(`OTP sent successfully to ${recipientPhoneNumber}`);
        ...
        console.error('Error handling /otp request:', error.message);
...
app.post('/validate-otp', async (req, res) => {
    try {
        ...
        console.error('Error handling /validate-otp request:', error.message);
        res.status(500).json({ error: 'Internal server error' });
    }
});
